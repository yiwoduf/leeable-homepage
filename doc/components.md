...
| `ContactSection` | Big line, `EmailCopy` codebox, GitHub/LinkedIn/X/Résumé buttons and a primary meeting button in one action group (pointer glow). At ≤960px, links form two columns and the meeting button spans the group width. Footer with © line and a GitHub view-source icon next to "Built in public". |
